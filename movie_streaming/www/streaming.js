...
const isCapacitorApp = typeof Capacitor !== 'undefined' && Capacitor.isNativePlatform && Capacitor.isNativePlatform();

// Capacitor Plugins (loaded dynamically when in native app)
let StatusBar = null;
...
async function initCapacitor() {
    ...
    try {
        ...
        const { ScreenOrientation: SO } = await import('@capacitor/screen-orientation');
        ...
        console.log('[Capacitor] Running in web mode or plugins not available:', error.message);
    }
}

// Handle Android hardware back button
async function handleAndroidBackButton({ canGoBack }) {
    // Check if watch page is open
    const watchPage = document.getElementById('watch-page');
    const isWatchPageVisible = watchPage && watchPage.style.display !== 'none';
    ...
    // If on home page, exit app (with confirmation)
    if (canGoBack) {
        window.history.back();
    } else {
...
// Toggle video fullscreen mode with Capacitor support
async function toggleVideoFullscreen(videoContainer) {
    const isFullscreen = document.fullscreenElement || document.webkitFullscreenElement;
    ...
    if (!isFullscreen) {
        ...
        if (isCapacitorApp && ScreenOrientation) {
            try {
        ...
        if (hash) {
            ...
            if (mediaType && mediaId) {
                if (mediaType === 'tv' && season && episode) {
                    showWatchPage(mediaId, mediaType, parseInt(season), parseInt(episode));
                } else {
                    showWatchPage(mediaId, mediaType);
                }
            }
        }
    } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add double-tap fullscreen for video
    const watchIframe = document.getElementById('watch-iframe');
    if (watchIframe) {
        let lastTap = 0;
        watchIframe.parentElement.addEventListener('touchend', (e) => {
            const currentTime = new Date().getTime();
            const tapLength = currentTime - lastTap;
            if (tapLength < 500 && tapLength > 0) {
                toggleVideoFullscreen(watchIframe.parentElement);
                e.preventDefault();
            }
            lastTap = currentTime;
        });
...
// ============================================
// End of Capacitor Integration
// ============================================

// ============================================
// Ad Blocking Configuration
// ============================================
const BLOCK_ADS = false; // Set to true to block server-side ads
...
const STREAM_PROVIDERS = [
    ...
    },
    {
        ...
        id: 'vidlink',
        getUrl: (type, id, s, e) => {
            if (type === 'movie') return `https://vidlink.pro/movie/${id}?primaryColor=dc2626&secondaryColor=ffffff&iconColor=dc2626&autoplay=true`;
            return `https://vidlink.pro/tv/${id}/${s}/${e}?primaryColor=dc2626&secondaryColor=ffffff&iconColor=dc2626&autoplay=true`;
        }
    },
    {
        name: 'Server 3',
    ...
    serverContainer.innerHTML = `
        <div class="flex items-center gap-3 overflow-x-auto no-scrollbar pb-2">
            ...
            ${STREAM_PROVIDERS.map((provider, index) => `
                <button 
                    class="server-btn min-w-[80px] px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${provider.id === currentProvider.id ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}"
                    data-id="${provider.id}">
                    Server ${index + 1}
                </button>
            `).join('')}
...
function updateVideoSource(mediaId, mediaType) {
    ...
    // Ad blocking: Intercept first few clicks on iframe to prevent ad redirects
    if (BLOCK_ADS) {
        setupAdBlocker(watchIframe);
    }
    
    watchIframe.src = url;
}

// Ad Blocker: Intercepts the first click on the video player to absorb ad redirects
let adClicksBlocked = 0;
const MAX_AD_CLICKS_TO_BLOCK = 2;

function setupAdBlocker(iframe) {
    adClicksBlocked = 0;
    
    // Remove existing overlay if any
    const existingOverlay = iframe.parentElement.querySelector('.ad-click-blocker');
    if (existingOverlay) existingOverlay.remove();
    
    // Create transparent click overlay
    const overlay = document.createElement('div');
    overlay.className = 'ad-click-blocker';
    ...
    overlay.addEventListener('click', (e) => {
        adClicksBlocked++;
        console.log(`🚫 Ad click blocked (${adClicksBlocked}/${MAX_AD_CLICKS_TO_BLOCK})`);
        
        if (adClicksBlocked >= MAX_AD_CLICKS_TO_BLOCK) {
            // After blocking enough clicks, remove overlay to allow normal playback
            overlay.remove();
            console.log('✅ Ad blocker deactivated - video should play normally now');
        }
    ...
    // Auto-remove overlay after 5 seconds if user doesn't click
    setTimeout(() => {
        if (overlay.parentElement) {
            overlay.remove();
            console.log('✅ Ad blocker auto-deactivated after timeout');
        }
    }, 5000);
